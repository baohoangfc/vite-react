...
export default function MarketRadar({ analysis }: MarketRadarProps) {
    return (
        <div className="bg-slate-800 p-5 rounded-2xl border border-slate-700 relative overflow-hidden shadow-lg">
            ...
            <div className="space-y-4">
                <div>
                    ...
                    <div className="text-center mt-1 text-xs font-bold text-slate-100">Điểm Đồng thuận: {analysis.score > 0 ? '+' + analysis.score : analysis.score}/5</div>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-3 border-t border-slate-700">
                    <div className="bg-slate-900 p-3 rounded-xl border border-slate-700">
                        <span className="text-[10px] text-slate-400 uppercase font-bold block mb-1">Xu Hướng (EMA)</span>
                    ...
                    <div className="bg-slate-900 p-3 rounded-xl border border-slate-700">
                        <span className="text-[10px] text-slate-400 uppercase font-bold block mb-1">RSI (14) Động</span>
                        <span className={`text-sm font-black ${analysis.rsi > 65 ? 'text-red-400' : analysis.rsi < 35 ? 'text-green-400' : 'text-slate-100'}`}>
                            {analysis.rsi.toFixed(1)}
                        </span>
                    </div>
                    <div className="bg-slate-900 p-3 rounded-xl border border-slate-700 text-center sm:text-left">
                        <span className="text-[10px] text-slate-400 uppercase font-bold block mb-1">Dòng tiền SMC</span>
                    ...
                    <div className="bg-slate-900 p-3 rounded-xl border border-slate-700 text-center sm:text-left">
                        <span className="text-[10px] text-slate-400 uppercase font-bold block mb-1">Động lượng MACD</span>
