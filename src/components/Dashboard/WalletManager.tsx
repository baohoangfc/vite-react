...
export default function WalletManager({ account, position, unrealizedPnl }: WalletManagerProps) {
    ...
    return (
        <div className="bg-slate-800 p-5 rounded-2xl border border-slate-700 relative shadow-lg">
            ...
            <div className="space-y-4">
                <div>
                    <span className="text-[10px] text-slate-400 font-bold uppercase block mb-1">Số dư Ví (USDT)</span>
                    <span className="text-3xl font-mono font-black text-slate-900">{account.balance.toFixed(2)}</span>
                </div>
                <div className="pt-3 border-t border-slate-700">
                    <span className="text-[10px] text-slate-400 font-bold uppercase block mb-1">Tài sản Ròng (Bao gồm PnL)</span>
