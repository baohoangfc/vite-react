...
export default function DailyAggregation({ history }: DailyAggregationProps) {
    ...
    return (
        <div className="space-y-3">
            ...
            {dailyStats.map((stat) => (
                <div key={stat.date} className="bg-white/5 p-4 rounded-xl border border-white/5 flex justify-between items-center transition-hover hover:bg-white/10">
                    <div>
                        <div className="text-xs font-black uppercase text-blue-400">{stat.date}</div>
                        <div className="text-[9px] text-gray-500 font-bold mt-1 uppercase tracking-widest">
                            {stat.trades} Lệnh • Winrate: {stat.winRate}%
                        </div>
                    </div>
                    <div className="text-right">
                        <div className={`text-sm font-black ${stat.pnl > 0 ? 'text-green-400' : 'text-red-400'}`}>
