...
export default function ActivePosition({
    ...
    return (
        <div className={`backdrop-blur-xl p-5 rounded-2xl border transition-all duration-500 flex flex-col h-full ${position ? 'bg-slate-200 border-blue-400/40 shadow-lg' : 'bg-slate-200 border-slate-300 opacity-90 shadow-lg'}`}>
            <h3 className="text-xs font-black text-slate-600 uppercase tracking-widest mb-4 flex justify-between items-center">
                <span className="flex items-center gap-2"><Zap size={14} className={position ? "text-yellow-400 animate-pulse" : "text-slate-500"} /> Vị thế Active</span>
            ...
            {position ? (
                <div className="space-y-4 flex-1 flex flex-col justify-between">
                    <div className="text-center bg-slate-100 p-3 sm:p-4 rounded-xl border border-slate-300 shadow-inner">
                        <span className="text-[10px] text-slate-500 font-bold uppercase block mb-1">Lợi nhuận Tạm tính (ROE)</span>
                    ...
                    <div className="grid grid-cols-2 gap-2 sm:gap-3 text-xs">
                        <div className="bg-slate-100 p-2 rounded-lg border border-slate-300 text-center">
                            <span className="block text-[9px] text-slate-500 uppercase font-bold mb-0.5">Vào Lệnh</span>
                            <span className="text-slate-700 font-mono font-bold text-[10px] sm:text-xs">{position.entryPrice.toLocaleString()}</span>
                        </div>
                        <div className="bg-slate-100 p-2 rounded-lg border border-slate-300 text-center">
                            <span className="block text-[9px] text-slate-500 uppercase font-bold mb-0.5">Ký quỹ</span>
                            <span className="text-slate-700 font-mono font-bold text-[10px] sm:text-xs">${position.margin.toFixed(1)}</span>
                        </div>
                    </div>
                    ...
                    <div className="flex justify-between text-[9px] font-bold uppercase text-slate-500 px-1">
                ...
                <div className="flex-1 flex flex-col items-center justify-center text-center opacity-50">
                    <Target size={40} className="text-slate-500 mb-3" />
                    <p className="text-xs font-bold text-slate-500 uppercase tracking-widest">Hệ thống đang rình mồi...</p>
                    <p className="text-[10px] text-slate-500 mt-1 max-w-[200px]">AI đang phân tích cấu trúc SMC đa chỉ báo để tìm điểm vào an toàn nhất.</p>
                </div>
            )}
        </div>
    );
